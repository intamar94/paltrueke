...
const LEGAL_PATHS = ["/privacidad", "/terminos", "/seguridad", "/contacto"];
...
export default function App() {
  ...
  const [legalPath, setLegalPath] = useState(() => LEGAL_PATHS.includes(window.location.pathname) ? window.location.pathname : null);
  ...
  useEffect(() => { const onPopState = () => { const path = LEGAL_PATHS.includes(window.location.pathname) ? window.location.pathname : null; setLegalPath(path); if (!path) setView("home"); }; window.addEventListener("popstate", onPopState); return () => window.removeEventListener("popstate", onPopState); }, []);
  useEffect(() => { topRef.current?.scrollIntoView({ behavior: "auto", block: "start" }); }, [view, legalPath]);

  const navigateLegal = (path) => { setLegalPath(LEGAL_PATHS.includes(path) ? path : null); };
  const backFromLegal = () => { window.history.pushState({}, "", "/"); setLegalPath(null); setView("home"); };
  ...
  return <div ref={topRef} style={{ minHeight: "100vh" }}>
    ...
    {legalPath ? <><LegalView path={legalPath} onBack={backFromLegal} onNavigate={navigateLegal} /><Footer onNavigate={navigateLegal} /></> : <>
