...
export default function App() {
  ...
  return <div style={{ minHeight: "100vh" }}>
    ...
    {pushOffer && <div style={{ position: "fixed", left: "50%", bottom: 92, transform: "translateX(-50%)", width: "min(420px, calc(100% - 32px))", zIndex: 45, background: "#fff", border: "1px solid var(--border)", borderRadius: 18, padding: "14px 16px", boxShadow: "0 10px 28px rgba(74,51,40,0.18)" }}><div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}><Bell size={19} color="var(--naranja)" style={{ flexShrink: 0, marginTop: 2 }} /><div style={{ flex: 1 }}><strong style={{ display: "block", color: "var(--ink)", fontSize: 14, marginBottom: 4 }}>¿Quieres enterarte de novedades?</strong><span style={{ display: "block", color: "var(--muted)", fontSize: 12.5, lineHeight: 1.4 }}>Puedes activar las notificaciones de Pa'l Trueke para recibir avisos importantes. Es opcional.</span><div style={{ display: "flex", gap: 8, marginTop: 10 }}><button onClick={enablePush} style={{ border: "none", borderRadius: 12, padding: "8px 12px", background: "var(--naranja)", color: "#fff", fontWeight: 700, fontSize: 12.5 }}>Activar</button><button onClick={dismissPushOffer} style={{ border: "1px solid var(--border)", borderRadius: 12, padding: "8px 12px", background: "#fff", color: "var(--muted)", fontWeight: 600, fontSize: 12.5 }}>Ahora no</button></div></div></div></div>}
