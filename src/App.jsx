...
export default function App() {
  ...
  const enterFeed = useCallback(async (tipoFiltro, title, nextFilters = null) => {
    setFeedEntering(true);
    if (nextFilters) setFilters(nextFilters);
    else setFilters({ ...EMPTY_FILTERS, tipo: tipoFiltro || "todo" });
    setFeedTitle(title || "Inicio");
    setView("feed");
    try {
      await reload();
    } finally {
      setFeedEntering(false);
    }
  }, [reload]);
  ...
  const createPost = useCallback(
    async (draft) => {
      ...
      if (err) {
        const motivo = err.message?.includes("3 pedidos") ? "3 pedidos" : err.message?.includes("6 publicaciones") ? "6 publicaciones" : null;
        if (motivo) {
          try { await supabase.from("blocked_attempts").insert({ owner_id: userId, motivo }); } catch (_) {}
        }
        throw err;
      }
      setFormTipo(null);
  ...
  const updatePost = useCallback(
    async (post, draft) => {
      const { error: err } = await supabase.from("posts").update({ tipo: draft.tipo, categoria: draft.categoria, titulo: catInfo(draft.categoria).label, descripcion: draft.descripcion.trim() || null, pais: "Colombia", departamento: draft.departamento.trim() || "Colombia", municipio: draft.municipio.trim(), sector: draft.sector.trim() || null, urgente: draft.urgente }).eq("id", post.id);
      if (err) throw err;
      setEditingPost(null);
  ...
  const resolve = async (post) => {
    ...
    if (post.helper_id) {
      ...
      message = otraParteYaConfirmo ? "Publicación resuelta. Gracias a los dos por confirmar." : "Tu confirmación quedó registrada. Falta que la otra persona también confirme.";
    }
    setToast(message); await reload(true); setConfirming(null);
  };
  const releasePost = async (post) => { setActionError(""); const { error: err } = await supabase.rpc("release_helper", { p_id: post.id }); if (err) { setActionError(err.message); setConfirming(null); return; } setToast("Publicación liberada, vuelve a estar disponible para que alguien más ayude."); await reload(true); setConfirming(null); };
